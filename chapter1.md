# Chapter 1: xrt Training Environment and Calculations
...
This installs all required dependencies and prepares the environment. The Pixi manifest should install xrt with the `all` optional dependencies, written as `xrt[all]`, so the same environment will also work for the GUI and controls sections in Chapter 2.
...
This will install all required dependencies and create the Pixi environment used by the project. Make sure the manifest installs xrt as `xrt[all]`, not plain `xrt`; Chapter 2 uses the optional GUI and controls packages.
